...
public static class DbInitializer
{
    ...
    {
        // Check if offers exist, if not, seed them
        if (!await context.Offers.AnyAsync())
        {
            await SeedOffersAsync(context);
        }
    ...
    {
        // Get existing restaurants with their menu items
        ...
        {
            return; // Cannot seed offers without restaurants
        }

        var offers = new List<Offer>();

        // For each restaurant, get its first menu item and create an offer
        int index = 0;
        foreach (var restaurant in restaurants)
        {
            ...
            if (menuItem != null)
            {
                var discountPercent = 15.00m + (index % 3) * 5.00m; // 15%, 20%, 25%
                var daysValid = 3 + (index % 5); // 3-7 days

                offers.Add(new Offer
                {
                    Id = Guid.NewGuid(),
                    Title = $"{restaurant.Name} Xüsusi Təklif",
                    Description = $"Seçilmiş məhsullarda {discountPercent:F0}% endirim!",
                    DiscountPercent = discountPercent,
                    StartAt = DateTimeOffset.UtcNow,
                    EndAt = DateTimeOffset.UtcNow.AddDays(daysValid),
                    RestaurantId = restaurant.Id,
                    MenuItemId = menuItem.Id,
                    IsPersonalized = index % 2 == 0,
                    IsActive = true
                });
            }
            index++;
